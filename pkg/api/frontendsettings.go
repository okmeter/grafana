...
import (
	op_frontend "github.com/grafana/grafana/op-pkg/frontend"
)
...
func (hs *HTTPServer) getFrontendSettings(c *contextmodel.ReqContext) (*dtos.FrontendSettingsDTO, error) {
	...
	// OP_CHANGES.md: override appURL and appSubURL for frontend
	appURL, appSubURL := op_frontend.BuildAppURLOverrides(c.Req.Context(), hs.Cfg.AppURL)
	...
	frontendSettings := &dtos.FrontendSettingsDTO{
		...
		Panels:                              panels,
		Apps:                                apps,
		AppUrl:                              appURL,    // original: hs.Cfg.AppURL
		AppSubUrl:                           appSubURL, // original: hs.Cfg.AppSubURL
