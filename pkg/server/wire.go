...
import (
	op_encryption "github.com/grafana/grafana/op-pkg/service/encryption"
	op_store "github.com/grafana/grafana/op-pkg/store"
)
...
var wireBasicSet = wire.NewSet(
	...
	// original: encryptionservice.ProvideEncryptionService
	// original: wire.Bind(new(encryption.Internal), new(*encryptionservice.Service))
	// OP_CHANGES.md: use custom encryption
	op_encryption.ProvideEncryptionService,
	wire.Bind(new(encryption.Internal), new(*op_encryption.Service)),
	...
	// original: wire.Bind(new(folder.FolderStore), new(*folderimpl.DashboardFolderStoreImpl))
	// OP_CHANGES.md: use custom dashboard/folder store
	wire.Bind(new(folder.FolderStore), new(*op_store.DashboardStore)),
