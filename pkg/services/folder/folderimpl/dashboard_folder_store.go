...
// ProvideDashboardFolderStore(sqlStore db.DB) *DashboardFolderStoreImpl
func ProvideDashboardFolderStore(sqlStore db.DB) *op_store.DashboardStore {
	// original: return &DashboardFolderStoreImpl{store: sqlStore}
	// OP_CHANGES.md: use custom dashboard/folder store
	return op_pkg.GetDashboardStore(log.New("dashboard-folder-store"))
}
