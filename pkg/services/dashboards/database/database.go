...
func ProvideDashboardStore(sqlStore db.DB, cfg *setting.Cfg, features featuremgmt.FeatureToggles, tagService tag.Service, quotaService quota.Service) (dashboards.Store, error) {
	// original: s := &dashboardStore{store: sqlStore, cfg: cfg, log: log.New("dashboard-store"), features: features, tagService: tagService}
	// OP_CHANGES.md: use custom dashboard/folder store
	s := op_pkg.GetDashboardStore(log.New("dashboard-store"))
